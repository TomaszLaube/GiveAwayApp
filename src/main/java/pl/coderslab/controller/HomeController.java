...
public class HomeController {
    ...
    public String registeredUser(@ModelAttribute @Valid User user, BindingResult result, Model model) {
        ...
        if (result.hasErrors()) {
            ...
            try{
                MailSender.activateAccount(user.getEmail(),user.getFirstName(),user.getLastName(), userUUID.getRegUUID());
    ...
    public String registeredOrganisation(@ModelAttribute @Valid User user, BindingResult result, Model model) {
        ...
        if (result.hasErrors()) {
            ...
            uuidService.save(userUUID);
            try{
                MailSender.activateAccount(user.getEmail(),user.getName(),"", userUUID.getRegUUID());
            } catch (AddressException e){
                e.printStackTrace();
            } catch (MessagingException e){
                e.printStackTrace();
    ...
    @RequestMapping("/activateAccount/{regUuid}")
    public String activateAccount(@PathVariable UUID regUuid) {
        pl.coderslab.model.UUID userUuid = (pl.coderslab.model.UUID) uuidService.findByRegUUID(regUuid);
        if (userUuid != null) {
    ...
    public String contactForm(@RequestParam String email, @RequestParam String firstName, @RequestParam String lastName, @RequestParam String message, Model model) {
        try{
            MailSender.contactForm("HeadBeatMailing@gmail.com",email,firstName,lastName,message);
        } catch (AddressException e){
            e.printStackTrace();
        } catch (MessagingException e){
            e.printStackTrace();
    ...
    public String clearedPwd(@RequestParam String email, Model model) {
        ...
        if (byEmail != null) {
            ...
            if (userUUID != null) {
                ...
                try{
                    MailSender.resetPwd(email,userUUID.getPwdUUID());
                } catch (AddressException e){
                    e.printStackTrace();
                } catch (MessagingException e){
                    e.printStackTrace();
